...
if ($_SERVER["REQUEST_METHOD"] !== "POST") { header("Location: complaints.php"); exit(); }
csrf_verify_or_die();

$complaint_id = (int)($_POST["complaint_id"] ?? 0);
$status = trim($_POST["status"] ?? "");
$allowed_status = ["Pending", "Under Investigation", "Resolved"];
if ($complaint_id <= 0 || !in_array($status, $allowed_status, true)) { header("Location: complaints.php"); exit(); }
...
if (db_table_exists($conn, "notifications")) {
    $notif_stmt = mysqli_prepare($conn, "INSERT INTO notifications (user_id, complaint_id, message, type, is_read) VALUES (?, ?, ?, 'status', 0)");
